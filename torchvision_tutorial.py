...
## NOTE: in docker container, there is no wget!
...
dataset = PennFudanDataset('test/fixture/ipa', get_transform(train=True))
dataset_test = PennFudanDataset('test/fixture/ipa', get_transform(train=False))
...
boxed = draw_bounding_boxes(image, pred_boxes, pred_labels)

masks = (pred["masks"] > 0.7).squeeze(1)
masked = draw_segmentation_masks(image, masks, alpha=0.5)

from datetime import datetime
timestamp = datetime.now().strftime('%Y%m%d-%H%M%S')

from torchvision.io import write_png
#write_png(boxed, f'out/{timestamp}.boxes.png')
write_png(masked, f'out/{timestamp}.masks.png')
